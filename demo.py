...
def main(model, vc):
        ...
        while rval:
            ...
            for contour in contours:
                ...
                if 5 < w * h < 10000:  # Adjust size range as needed
                    ...
                    cv2.imshow("yolo", yolo(model, frame))
                    # cv2.imshow("tensor", tensorrt(frame))
    ...
    avg_time_per_frame = total_time / frame_count  # Calculate the average time per frame
...
def yolo(model, frame):
    with torch.no_grad():
        result, names = model.detect([frame])
        img = result[0][0]  # 每一帧图片的处理结果图片
        # 每一帧图像的识别结果（可包含多个物体）
        for cls, (x1, y1, x2, y2), conf in result[0][1]:
            print(names[cls], x1, y1, x2, y2, conf)  # 识别物体种类、左上角x坐标、左上角y轴坐标、右下角x轴坐标、右下角y轴坐标，置信度
...
def tensorrt(frame):
    frame = infer.detect(frame)
    return frame


if __name__ == '__main__':
    vc = cv2.VideoCapture('smoke_viedo/Black_smoke_517.avi')  # 0
    a = detect_api.detectapi(weights='models/best.pt')
    main(a, vc)
